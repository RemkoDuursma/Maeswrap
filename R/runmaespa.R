...
function(whichrow=1, whichcols=NA, runfile=file.choose(), runit=TRUE, executable="maespa.exe",
  ...
  simsuccess <- function(){
  	...
  	st <-r[length(r)]
  	length(grep("SIMULATION SUCCESSFULLY COMPLETED", st)) > 0
  }


  defdata <- utils::read.table(deffile, fill=TRUE, header=TRUE, colClasses="character")
